...
function mapCompanyLogosToUseCases(useCasesData: any[], allFile: LogoFile[]): any[] {
...
export const createUseCases = async ({ createPage, graphql }: CreateUseCasesArgs) => {
    ...
    useCasesWithLogos.forEach((node, index) => {
        ...
        const relatedArticleIndices = generateRandomIndices(useCasesWithLogos.length, 8); // 8 is the number of related articles to show
        ...
        createPage({
          path: `/use-cases/${getSlug(node.Case_Name)}`,
